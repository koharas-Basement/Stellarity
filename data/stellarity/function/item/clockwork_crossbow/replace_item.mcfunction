...
execute if predicate stellarity:item/holding/clockwork_crossbow/unloaded_mainhand run \
	item modify entity @s weapon.mainhand stellarity:load_clockwork_crossbow
execute if predicate stellarity:item/holding/clockwork_crossbow/unloaded_offhand run \
	item modify entity @s weapon.offhand stellarity:load_clockwork_crossbow

# Ammo conservation
execute if predicate stellarity:item/clockwork_crossbow/quick_charge/0 \
	if predicate kohara:chance/25percent run \
	scoreboard players set @s stellarity.items.clockwork_crossbow.should_save 1
execute if predicate stellarity:item/clockwork_crossbow/quick_charge/1 \
	if predicate kohara:chance/31percent run \
	scoreboard players set @s stellarity.items.clockwork_crossbow.should_save 1
execute if predicate stellarity:item/clockwork_crossbow/quick_charge/2 \
	if predicate kohara:chance/37percent run \
	scoreboard players set @s stellarity.items.clockwork_crossbow.should_save 1
execute if predicate stellarity:item/clockwork_crossbow/quick_charge/3_and_more \
...
execute at @s run function stellarity:item/clockwork_crossbow/sounds
