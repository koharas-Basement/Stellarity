data modify storage stellarity:temp enchants set from entity @e[type=item,tag=stellarity.altar_of_the_accursed.netherite_shovel,distance=..1.5,limit=1] Item.tag.Enchantments

kill @e[type=item,tag=stellarity.altar_of_the_accursed.shulker_shells,distance=..1.5,limit=1]
kill @e[type=item,tag=stellarity.altar_of_the_accursed.netherite_shovel,distance=..1.5,limit=1]

loot spawn ~ ~ ~ loot stellarity:items/tools/shulker/shovel
